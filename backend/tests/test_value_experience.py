...
<footer>base Avoulia v4.6.3 — cas UC-0001.</footer></body>"""
        ...
        self.assertEqual(result.count("Conserver aussi ce second paragraphe."), 1)
        self.assertEqual(evidence["case_id"], "UC-0001")
        self.assertEqual(result.count('id="bilan-temps"'), 1)
        self.assertNotIn("lien-retour", result)
        self.assertNotIn("copier-lien", result)
        self.assertIn("select.selectedIndex=0", result)
